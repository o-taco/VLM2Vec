...
# best combined_and per (window) -- pick the tau grid, tag with window
rows = []
...
Path("outputs/adaptive_depth/comparison.json").write_text(json.dumps(comparison, indent=2))

for name, rows in comparison.items():
    print(f"\n=== {name} vs. interpolated fixed-depth baseline ===")
    for r in rows:
