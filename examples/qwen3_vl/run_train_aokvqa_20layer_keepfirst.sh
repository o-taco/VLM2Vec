#!/bin/bash
# Contiguous-drop counterpart to run_train_aokvqa_20layer.sh: keep the first
# 20 layers (0..19), drop the contiguous tail, instead of the alternating
...
python train.py \
  --model_name pruned_models/qwen3vl-2b-20layer-keepfirst \
  ...
  --output_dir output/aokvqa_qwen3vl_20layer_keepfirst \
